...
function wrap(text: string, max: number = W - 4): string {
  const out: string[] = [];
  for (const para of text.split("\n")) {
    if (para.length <= max) { out.push(para); continue; }
    let r = para;
    while (r.length > max) { out.push(r.slice(0, max)); r = r.slice(max); }
    if (r) out.push(r);
  }
  return out.join("\n");
}
...
function spinnerStart(text: string) {
  spinnerStop();
  spinnerText = text;
  spinnerActive = true;
  spinnerIdx = 0;
...
function spinnerUpdate(text: string) {
  spinnerText = text;
}

function spinnerStop(final?: string) {
  if (spinnerTimer) { clearInterval(spinnerTimer); spinnerTimer = null; }
  if (spinnerActive) {
    process.stdout.write("\r\x1b[K"); // 清除当前行
    if (final) console.log(C.muted(`  ${final}`));
  }
  spinnerActive = false;
}
...
/** 模型思考内容 */
function renderLLMOutput(content: string) {
...
/** 工具执行结果（⎿ 缩进风格，对齐官方） */
function renderToolResult(tool: string, result: string) {
  const meta = TOOL_META[tool] ?? { icon: "🔧", label: tool, color: C.tool };
...
/** 工具状态指示器（◌ 开始 / ○ 完成） */
function renderToolStatus(tool: string, done: boolean) {
  const meta = TOOL_META[tool] ?? { icon: "🔧", label: tool, color: C.tool };
...
/** 最终回答 */
function renderFinalAnswer(content: string) {
  divider("结果");
...
/** 轮次标题 */
function renderRound(n: number) {
  if (n > 1) console.log(""); // 轮次间空行
...
async function main() {
  ...
  // ---- 启动 Python 持久进程 ----
  const proc: ChildProcess = spawn("python", [PY_SCRIPT, "--json"], {
    cwd: ROOT,
  ...
  proc.stdout!.on("data", (chunk: Buffer) => {
    ...
    for (const line of lines) {
      ...
      try {
        handle(JSON.parse(line) as JsonEvent);
      } catch {
  ...
  let inputBuf = "";
  let inputResolve: ((s: string) => void) | null = null;
  ...
  while (true) {
    ...
    await new Promise<void>((r) => { currentResolve = r; });
    console.log("");
  }
